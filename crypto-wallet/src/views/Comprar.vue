...
const cryptoAmountdeCompra = ref('')
const dateTime = ref('')
const mensaje = ref('')
const error = ref('')
...
async function enviar() {
  ...
  if (!cryptoAmountdeCompra.value || cryptoAmountdeCompra.value <= 0) {
  ...
  const guardavalor = {
    ...
    action: 'purchase',
    CryptoAmount: parseFloat(cryptoAmountdeCompra.value),
  ...
  try {
    const res = await fetch('https://localhost:7278/api/transacciones', {
      ...
      body: JSON.stringify(guardavalor)
    })

    if (!res.ok) {
      error.value = 'Error al guardar la compra'
      return
    }

    mensaje.value = 'Compra registrada correctamente'
    cryptoAmountdeCompra.value = ''
    dateTime.value = ''
...
<template>
  <div class="contenedor">
    ...
    <div class="campo">
      ...
      <input type="number" step="0.00000001" v-model="cryptoAmountdeCompra" placeholder="0.00000001" />
    </div>
    ...
    <button @click="enviar">Confirmar Compra</button>
